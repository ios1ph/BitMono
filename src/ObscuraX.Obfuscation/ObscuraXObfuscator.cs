namespace ObscuraX.Obfuscation;

public class ObscuraXObfuscator
{
        ...
        ILogger logger)
    {
        ...
        _logger = logger.ForContext<ObscuraXObfuscator>();
    ...
    public async Task ProtectAsync()
    {
        ...
        await _pipeline.InvokeAsync(OutputObscuraXInfo);
    ...
    private void OutputObscuraXInfo()
    {
    ...
    /// but ObscuraX is running on .NET Core, or vice versa.
    /// See more info: https://obscurax.readthedocs.io/en/latest/obfuscationissues/corlib-not-found.html
    /// </summary>
    private void OutputCompatibilityIssues()
    {
        ...
        if (targetAssemblyRuntime.IsNetCoreApp && DotNetRuntimeInfoEx.IsNetFramework())
        {
            _logger.Warning(
                ...
                " To avoid potential issues, ensure the target framework matches the ObscuraX framework, " +
                "or switch to a .NET Core build of ObscuraX.");
            return;
        }
        if (targetAssemblyRuntime.IsNetFramework && DotNetRuntimeInfoEx.IsNetCoreOrLater())
        {
            _logger.Warning(
                "The module is built for .NET Framework, but you're using a version of ObscuraX intended for .NET (Core)." +
                " To avoid potential issues, ensure the target framework matches the ObscuraX framework, " +
                "or switch to a .NET Framework build of ObscuraX.");
            return;
        }
    }
    ...
    private bool ResolveDependencies()
    {
        ...
        var assemblyResolve = AssemblyResolver.Resolve(_context.ObscuraXContext.ReferencesData, _context);
    ...
    private async Task<bool> WriteModuleAsync()
    {
        try
        {
            ...
            await _dataWriter.WriteAsync(_context.ObscuraXContext.OutputFile, memoryStream.ToArray());
            _logger.Information("The protected module was saved in {0}", _context.ObscuraXContext.OutputDirectoryName);
        }
        catch (Exception ex)
        {
