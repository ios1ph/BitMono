...
{
    [CustomEditor(typeof(ObscuraXConfig))]
    public class ObscuraXConfigInspector : UnityEditor.Editor
    {
        private void OnEnable()
        {
            var config = (ObscuraXConfig)target;
        ...
        {
            var config = (ObscuraXConfig)target;
            EditorGUILayout.LabelField("ObscuraX Obfuscation", EditorStyles.boldLabel);
            config.EnableObfuscation = EditorGUILayout.Toggle(
                new GUIContent("Enable Obfuscation",
                    "Enable ObscuraX obfuscation during Unity builds. " +
                    "You can also control this programmatically using EditorPrefs.SetBool(\"ObscuraX.Obfuscation\", true/false). " +
            ...
            {
                EditorGUILayout.HelpBox("Auto detection failed. Please set a custom config path or ensure ObscuraX.CLI is accessible.", MessageType.Warning);
            }
            ...
            config.EnableDebugLogging = EditorGUILayout.Toggle(
                new GUIContent("Enable Debug Logging",
                    "Show detailed ObscuraX output in the Unity Console during builds. " +
            ...
            else
            {
                ...
                EditorGUILayout.HelpBox(
                    "Unity UI is disabled. ObscuraX will use protections.json file directly. " +
            ...
            EditorGUILayout.Space();
            EditorGUILayout.HelpBox(
                "ObscuraX will automatically detect configuration files from ObscuraX.CLI location. " +
